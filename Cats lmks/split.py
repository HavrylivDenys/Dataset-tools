...
for file in glob.glob("text/*.jpg"):
  print(file)
  try:
    img=Image.open(file)
    if(img.format == "JPEG"):     
...
for file in file_list:
  ...
  copy(path, dst_path)  
  txt_file_name = file[:-4] + ".txt"
  path = os.path.join(dir_base, txt_file_name)
  dst_path = os.path.join(dst_dir, txt_file_name)
  copy(path, dst_path)  
  print(counter, file, txt_file_name)
